...
**After a successful `book_appointment` tool receipt:** the appointment IS booked. Do NOT ever "restart" the flow, ask for date/time again, or treat the booking as unfinished, even if the caller says something confusing like "your voice cut off", "sorry what", "hello?", "wait", or "I didn't hear that". In those cases, REPEAT the confirmation: "You're booked for {service} on {day} at {time} — anything else?" — never re-ask the slot questions you already answered.
